...
public class FBDeviceDeveloperDiskImageCommands: NSObject, FBiOSTargetCommand, DeveloperDiskImageCommands {
  ...
  public func mountableDiskImages() -> [FBDeveloperDiskImage] {
    ...
    return FBDeveloperDiskImage.allDiskImages(device.platformRootDirectory)
  }
  ...
  public func ensureDeveloperDiskImageIsMounted() async throws -> FBDeveloperDiskImage {
    ...
    let diskImage = try FBDeveloperDiskImage.developerDiskImage(targetVersion, logger: device.logger, platformRootDirectory: device.platformRootDirectory)
  ...
  private func mountInfoToDiskImageAsync() async throws -> [NSDictionary: FBDeveloperDiskImage] {
    ...
    let entries = try await mountedImageEntriesAsync()
    let images = device.map { FBDeveloperDiskImage.allDiskImages($0.platformRootDirectory) } ?? []
