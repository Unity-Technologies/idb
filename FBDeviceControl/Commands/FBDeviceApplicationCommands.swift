...
// Fork-local: @objc and no longer private, so Pram can read processIdentifier
// through UnityLaunchedApplication.
@objc(FBDeviceLaunchedApplication)
public class FBDeviceLaunchedApplication: NSObject, FBLaunchedApplication, UnityLaunchedApplication {
...
public class FBDeviceApplicationCommands: NSObject, FBiOSTargetCommand {
  ...
  /// Installs the application at `path`, using a shadow directory on the host so that
  /// only changed files are transferred to the device.
  ///
  /// - Parameters:
  ///   - path: the file path of the Application Bundle on the host.
  ///   - shadowDir: directory on the host used for the shadow copy. When nil, a
  ///     directory under the user's caches directory is used, so that deltas survive
  ///     between runs -- upstream's own delta directory is per-session.
  fileprivate func deltaInstallApplicationAsync(withPath path: String, andShadowDirectory shadowDir: String?) async throws {
    guard let device else {
      throw FBDeviceControlError().describe("Device is nil").build()
    }
    var cacheDirectory = shadowDir
    if cacheDirectory == nil {
      let paths = NSSearchPathForDirectoriesInDomains(.cachesDirectory, .userDomainMask, true)
      cacheDirectory = paths.first.map { ($0 as NSString).appendingPathComponent("idb") }
    }
    guard let cacheDirectory else {
      throw FBDeviceControlError.describe("Could not determine a shadow directory to use for a delta install").build()
    }
    ...
    let options: [String: Any] = [
      "PackageType": "Developer",
      "ShadowParentKey": URL(fileURLWithPath: cacheDirectory),
    ]
    try await withFBFutureContext(device.connectToDevice(withPurpose: "install")) { connectedDevice in
      device.logger?.log("Installing Application \(appURL)")
      let statistics = FBDeviceWorkflowStatistics(workflowType: "Install", logger: connectedDevice.logger)
  ...
  fileprivate func launchApplicationAsync(_ configuration: FBApplicationLaunchConfiguration) async throws -> any FBLaunchedApplication {
    // FBApplicationLaunchModeFailIfRunning has to be handled here: the underlying
    // launch only distinguishes between killing an existing process and attaching.
    if configuration.launchMode == .failIfRunning, let running = try? await processIDAsync(withBundleID: configuration.bundleID) {
      throw FBDeviceControlError.describe("Application \(configuration.bundleID) already running with pid \(running)").build()
    }
    return try await launchApplicationIgnoringCurrentStateAsync(configuration)
  }

  private func launchApplicationIgnoringCurrentStateAsync(_ configuration: FBApplicationLaunchConfiguration) async throws -> any FBLaunchedApplication {
    guard let device else {
      throw FBDeviceControlError().describe("Device is nil").build()
    }
    let pid: NSNumber
  ...
  // MARK: Fork-local FBFuture surface for Pram
  //
  // Pram's idb-interface is Objective-C++ and chains FBFutures, so the historical
  // selectors are kept here over the async implementations above.

  @objc(deltaInstallApplicationWithPath:andShadowDirectory:)
  public func deltaInstallApplication(withPath path: String, andShadowDirectory shadowDir: String?) -> FBFuture<NSNull> {
    fbFutureFromAsync { [self] in
      try await deltaInstallApplicationAsync(withPath: path, andShadowDirectory: shadowDir)
      return NSNull()
    }
  }
  ...
  public func launchApplication(_ configuration: FBApplicationLaunchConfiguration) -> FBFuture<FBDeviceLaunchedApplication> {
    fbFutureFromAsync { [self] in
      guard let launched = try await launchApplicationAsync(configuration) as? FBDeviceLaunchedApplication else {
        throw FBDeviceControlError.describe("The launched application was not an FBDeviceLaunchedApplication").build()
      }
      return launched
    }
  }
  ...
  private func pidToRunningProcessNameAsync() async throws -> [NSNumber: String] {
    ...
    return try await withFBFutureContext(device.startService("com.apple.os_trace_relay")) { connection in
      ...
      // The payload is keyed by process identifiers rendered as strings, not as
      // numbers; casting to [NSNumber: Any] silently yields an empty dictionary and
      // runningApplications then always comes back empty.
      ...
      for (processIdentifier, value) in payload {
        guard let contents = value as? [String: Any],
          ...
          let processIdentifierNumber = Int(processIdentifier).map(NSNumber.init(value:))
        else {
          continue
        }
        pidToRunningProcessName[processIdentifierNumber] = processName
      }
