...
// Fork-local: conforms to UnityFileContainer so Pram can call it.
// See FBControlCore/Unity/UnityObjCCompatibility.swift.
@objc(FBDeviceFileContainer)
public class FBDeviceFileContainer: NSObject, AsyncFileContainer, UnityFileContainer {
  ...
  // MARK: UnityFileContainer -- fork-local FBFuture surface for Pram

  public func copyFromHost(_ sourcePath: String, toContainer destinationPath: String) -> FBFuture<NSNull> {
  ...
  public func copyFromContainer(_ sourcePath: String, toHost destinationPath: String) -> FBFuture<NSString> {
    fbFutureFromAsync { [self] in
  ...
  public func createDirectory(_ directoryPath: String) -> FBFuture<NSNull> {
    fbFutureFromAsync { [self] in
      // Disambiguated from this method: the async requirement shares its name.
      try await (self as any AsyncFileContainer).createDirectory(directoryPath)
      return NSNull()
    }
  }
  ...
  public func remove(_ path: String) -> FBFuture<NSNull> {
    fbFutureFromAsync { [self] in
      // Disambiguated from this method: the async requirement shares its name.
      try await (self as any AsyncFileContainer).remove(path)
      return NSNull()
    }
  }

  public func contentsOfDirectory(_ path: String) -> FBFuture<NSArray> {
    fbFutureFromAsync { [self] in
...
// Fork-local: @objc, with the historical selector Pram calls.
...
public class FBDeviceFileCommands: NSObject, FBiOSTargetCommand {
  ...
  public func fileCommandsForContainerApplicationObjC(_ bundleID: String) -> FBFutureContext<FBDeviceFileContainer> {
    fileCommandsForContainerApplication(bundleID)
  }
