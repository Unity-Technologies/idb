...
public class FBDeviceSet: NSObject, FBiOSTargetSet, FBiOSTargetSetDelegate {
  ...
  public convenience init(logger: any FBControlCoreLogger, delegate: (any FBiOSTargetSetDelegate)?, ecidFilter: String?) throws {
    try self.init(
      ...
      logger: logger,
      delegate: delegate,
      ecidFilter: ecidFilter
    )
  }

  /// Advanced initializer that allows custom work queues to be supplied.
  /// This is useful e.g. for console applications which don't have the main queue
  /// continuously fed, or components embedded in systems that are not aware of the main
  /// dispatch queue.
  ///
  /// Pram calls this as a class method, which an @objc initializer does not provide, so
  /// the factory below is declared explicitly.
  public convenience init(workQueue: DispatchQueue, asyncQueue: DispatchQueue, logger: any FBControlCoreLogger, delegate: (any FBiOSTargetSetDelegate)?, ecidFilter: String?) throws {
  ...
  @objc(setWithWorkQueue:asyncQueue:logger:delegate:ecidFilter:error:)
  public class func set(workQueue: DispatchQueue, asyncQueue: DispatchQueue, logger: any FBControlCoreLogger, delegate: (any FBiOSTargetSetDelegate)?, ecidFilter: String?) throws -> FBDeviceSet {
    try FBDeviceSet(workQueue: workQueue, asyncQueue: asyncQueue, logger: logger, delegate: delegate, ecidFilter: ecidFilter)
  }
