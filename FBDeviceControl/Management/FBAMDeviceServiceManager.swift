...
public class FBAMDeviceServiceManager: NSObject {
  ...
  /// Invalidates the house arrest connection for a given bundle id.
  /// Does nothing if no connection exists already.
  /// This can be used whenever a cached connection returns previously encountered errors
  /// on subsequent operations.
  @objc(invalidateHouseArrestAFCConnectionForBundleID:)
  public func invalidateHouseArrestAFCConnection(forBundleID bundleID: String) {
    houseArrestManagers.removeValue(forKey: bundleID)
    houseArrestDelegates.removeValue(forKey: bundleID)
  }
}
