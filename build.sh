...
function invoke_xcodebuild() {
  ...
  # ARCHS defaults to arm64 only (no Intel/x86_64 slices). Fork-local: IDB_ARCHS
  # overrides it, so Unity's CI can build the universal frameworks it ships.
  local common_settings=(
    -skipMacroValidation
    ENABLE_USER_SCRIPT_SANDBOXING=NO
    CLANG_ENABLE_EXPLICIT_MODULES=NO
    ARCHS="${IDB_ARCHS:-arm64}"
  )
