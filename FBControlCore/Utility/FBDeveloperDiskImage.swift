...
public class FBDeveloperDiskImage: NSObject {
  ...
  @objc(developerDiskImage:logger:platformRootDirectory:error:)
  public class func developerDiskImage(_ targetVersion: OperatingSystemVersion, logger: (any FBControlCoreLogger)?, platformRootDirectory: String) throws -> FBDeveloperDiskImage {
    let images = allDiskImages(platformRootDirectory)
    return try bestImage(forImages: images, targetVersion: targetVersion, logger: logger)
  }

  /// The Disk Images available for the platform rooted at `platformRootDirectory`.
  /// The platform root is taken from the target rather than assumed to be iPhoneOS, so
  /// that tvOS and other platforms are not handed iOS images.
  @objc(allDiskImages:)
  public class func allDiskImages(_ platformRootDirectory: String) -> [FBDeveloperDiskImage] {
    _cachedAllDiskImagesLock.lock()
    defer { _cachedAllDiskImagesLock.unlock() }
    if let cached = _cachedAllDiskImages[platformRootDirectory] {
      return cached
    }
    let searchPath = (platformRootDirectory as NSString).appendingPathComponent("DeviceSupport")
    ...
    _cachedAllDiskImages[platformRootDirectory] = images
    return images
  }

  private static let _cachedAllDiskImagesLock = NSLock()
  // Guarded by _cachedAllDiskImagesLock.
  nonisolated(unsafe) private static var _cachedAllDiskImages: [String: [FBDeveloperDiskImage]] = [:]
