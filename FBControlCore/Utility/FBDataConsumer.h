...
// Fork-local: the protocols above are defined in Swift, so an Objective-C
// consumer importing this header alone cannot declare a conforming class.
// Pull in their declarations the way upstream's other stub headers do.
//
// Skipped while building idb itself: this header is imported early in the
// umbrella, before the Objective-C types the generated header refers to (such
// as FBProcessIO) have been declared. Consumers import the umbrella first, so
// the ordering holds for them.
#if !defined(IDB_BUILDING_FRAMEWORKS)
